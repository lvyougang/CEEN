...
dic = torch.load(model_save + '1615102499.pkl')
...
# logging.info(text.shape)
# logging.info(embedding(torch.tensor(text)).shape)
...
if shuffle:
    ...
    for epoch in range(max_epoch):
        ...
        for i in range(train_step):
            ...
            # #还应该计算个mask
            # # logging.info('out[0] {} law_labels {}'.format(out[0].shape,law_labels_input.shape))
            # loss1 = criterion(out[0],law_labels_input.long())
            # loss2 = criterion(out[1], accu_labels_input.long())
            # loss3 = criterion(out[2], time_labels_input.long())
            # loss4=criterion(out[3], keti_input.long())
            ...
            # loss6 = criterion(out[5], zhuguan_input.long())
            #
            # # loss=loss1+loss2+loss3+loss_rate*(loss4+loss5+loss6)
        ...
        #         logging.info('Step %d: loss = %.2f (%.3f sec)' % (i, total_loss, duration))
        #         losses = total_loss
        #         total_loss = 0.0
        #
        #         logging.info(graph_chose_total)
        #         graph_chose_total = 0.0
        #
        #         logging.info(ave_graph_acc / train_step)
        #         ave_graph_acc = 0.0
        ...
        for i in range(valid_step):
            ...
            # predic_law += list((torch.argmax(out[0], 1) == law_labels_input).cpu().numpy())
            # predic_accu += list((torch.argmax(out[1], 1) == accu_labels_input).cpu().numpy())
            # predic_time += list((torch.argmax(out[2], 1) == time_labels_input).cpu().numpy())
        ...
        # t_law=test_accuracy1 / test_step
        # t_accu=test_accuracy2 / test_step
        # t_term=test_accuracy3 / test_step
        # logging.info('taccuracy_law: {:.4f} taccuracy_accu: {:.4f} taccuracy_term: {:.4f}'.format(t_law,
        #                                                                                    t_accu,
        #                                                                                    t_term))
        if (tt[0] + tt[1] + tt[2]) >= (best_law + best_accu + best_term):
            best_law = tt[0]
            best_accu = tt[1]
            best_term = tt[2]
            best_epoch = epoch + 1
            torch.save(model.state_dict(), model_save + str(current_time) + 'encoder' + '.pkl')
            torch.save(env.state_dict(), model_save + str(current_time) + 'env' + '.pkl')
            agent.save(model_save + str(current_time) + 'agent' + '.pkl')

        logging.info('max accu:\nbest_epoch {} {:.2f}% {:.2f}% {:.2f}%'.format(best_epoch, best_law, best_accu, best_term))